...
export const useRecordsTable = (objectType, workspaceId, viewId, views) => {
  ...
  const draggableColumns = headerGroup.headers.filter(
    (header) =>
      !header.id.includes("select") && !header.id.includes("add-column"),
  );
  ...
  const handleDragEnd = (event) => {
    ...
    if (active && over && active.id !== over.id) {
      ...
      const activeIndex = draggableColumnIds.indexOf(activeColumnId);
      const overIndex = draggableColumnIds.indexOf(overColumnId);
